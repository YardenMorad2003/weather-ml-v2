...
- **Unresolvable anchor (`AnchorError`).** Two detection paths, both leading to a typed error response with did-you-mean suggestions:
  1. Parse-time fictional detection. The parser exposes an `anchor_is_fictional: bool` field that the LLM populates based on the canonical referent of the name (mythology, pop culture). This catches `"Atlantis but colder"`, `"Camelot but warmer"`, `"Olympus but sunnier"` *before* the resolver runs — the resolver alone wouldn't catch these because the geocoder happily returns Atlantis FL (pop. 2,000), Camelot NY, Olympus WA, treating them as real obscure-town anchors. The LLM is the right tool for the "famous referent" judgment that no rule can reliably make.
  2. Resolver-failure fallback. For names the parser tags as real but the resolver can't place (`"Hogwarts but sunny"`, typos), `AnchorError` still fires through the original code path. Together the two paths prevent the bug this guard was added to prevent: `"Hogwarts but sunny"` → Cairo at 47% similarity.
