"""PCA-based 2D climate explorer.

Fits PCA(2) on the StandardScaler'd, hemisphere-normalized city profiles.
Returns 2D coords per city plus per-PC loadings to help interpret axes.
"""
...
def _top_loadings(components_row: np.ndarray, k: int = 8) -> list[Loading]:
    ...
    Profile layout is (12, 8) = months x features, flattened.
    Note: hemisphere-normalized profiles mean "Jan" is actually "month 1
    of the cold-season-aligned calendar" for SH cities.
    """
...
def _label_axes(pc1_top: list[Loading], pc2_top: list[Loading]) -> tuple[str, str]:
        ...
        messages=[
            {
                ...
                "content": (
                    ...
                    "'Dry sunny year-round'."
                ),
            },
            {
                "role": "user",
                "content": f"PC1 top loadings: {fmt(pc1_top)}\nPC2 top loadings: {fmt(pc2_top)}",
            },
        ],
        response_format=_AxisLabels,
    )
    labels = completion.choices[0].message.parsed
    return labels.pc1, labels.pc2


def get_overview() -> PCAOverview:
    st = get_state()
    pca = st["pca"]
    coords = st["profiles_2d"]
    ...
    if "axis_labels" not in st:
        st["axis_labels"] = _label_axes(pc1_top, pc2_top)
    pc1_label, pc2_label = st["axis_labels"]
...
def project_text(db: Session, text: str) -> ProjectedPoint:
    ...
    project the resulting user vector into the 2D PCA space."""
    ...
    xy = pca.transform(user_scaled.reshape(1, -1))[0]
